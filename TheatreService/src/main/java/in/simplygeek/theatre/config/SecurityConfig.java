package in.simplygeek.theatre.config;
...
import in.simplygeek.theatre.constants.CommonConstant;
import in.simplygeek.theatre.utils.JwtRequestFilter;
		...
		http.cors().configurationSource((HttpServletRequest request) -> {
			...
			return config;
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers(HttpMethod.GET, "/").permitAll().requestMatchers(SWAGGER_UI)
						...
						.requestMatchers("/api/mb/**").hasAuthority(CommonConstant.ROLE_ADMIN)
